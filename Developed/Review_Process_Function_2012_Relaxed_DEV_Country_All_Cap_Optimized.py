...
def Index_Continuity(TopPercentage_Securities, TopPercentage, Segment: pl.Utf8, temp_Emerging, country, Standard_Index):
    ...
    # Check if there are at least 5 NON-NEW Companies
    if (Segment == "Developed") & (len(TopPercentage_Securities.filter(pl.col("Shadow_Company") == False)) < 5):
        ...
        # Stack the Frames
        temp_Emerging_Country = temp_Developed_Current.vstack(temp_Developed_Non_Current).with_columns(
            pl.lit("All_Cap").alias("Size"),
            pl.lit(False).alias("Shadow_Company")
        )

        if len(temp_Emerging_Country) >= (5 - len(TopPercentage_Securities)):

            # Keep the Securities needed to get the minimum number of Securities
            temp_Developed_Country = temp_Emerging_Country.sort("Free_Float_MCAP_USD_Cutoff", descending = True).head(5 - len(TopPercentage_Securities))
...
def Minimum_FreeFloat_Country(TopPercentage, temp_Country, Lower_GMSR, Upper_GMSR, date, country, Segment: pl.Utf8, Original_MCAP_Emerging):
    ...
    if (date == Starting_Date) | (len(Output_Standard_Index.filter(pl.col("Country") == country)) == 0):
        ...
        if len(temp_Developed.filter(pl.col("Country") == country)) >= 5:

            # Check number of Current Securities
            if len(TopPercentage_Securities.filter(pl.col("Shadow_Company") == False)) < 5:
                # Keep only Non-Shadow Securities
        ...
        if date == Starting_Date: 
           ...
           # Apply the Check on Turnover for all Components
            ...
            for country in temp_Developed_Aggregate.select(pl.col("Country")).unique().sort("Country").to_series():
                ...
                # Small Index #
                Dveloped_Small = temp_Developed.filter((~pl.col("Internal_Number").is_in(TopPercentage_Securities.select(pl.col("Internal_Number")))) & (
                ...
                Small_Index = Small_Index.vstack(Dveloped_Small)
...
# Store the Results
Standard_Index.write_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\V0_SAMCO\Developed\Output\AllCap_Index_Security_Level_{GMSR_Upper_Buffer}_{GMSR_Lower_Buffer}_" + current_datetime + ".csv")
Recap_Count_Standard.write_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\V0_SAMCO\Developed\Output\Recap_Count_AllCap_{Percentage}_ETF_Version_Coverage_Adjustment_{Coverage_Adjustment}_" + current_datetime + ".csv")
