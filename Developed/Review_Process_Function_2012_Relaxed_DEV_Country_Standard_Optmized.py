...
# ETFs SPDR-iShares
ETF = pl.read_csv(r"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\V0_SAMCO\Developed\ETF.csv", separator=";")

# AllCap Index Output
AllCapIndexOutput = pl.read_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\V0_SAMCO\Developed\Output\AllCap_Index_Security_Level_{GMSR_Upper_Buffer_AllCap}_{GMSR_Lower_Buffer_AllCap}_" + current_datetime + ".csv").select(
...
def Index_Continuity(TopPercentage_Securities, TopPercentage, Segment: pl.Utf8, temp_Emerging, country, Standard_Index):
    ...
    if (Segment == "Developed") & (len(TopPercentage_Securities.filter(pl.col("Shadow_Company") == False)) < 5):
        ...
        temp_Developed_Country = temp_Developed.filter(pl.col("Country")==country).sort("Free_Float_MCAP_USD_Cutoff", descending=True)
        ...
        temp_Developed_Current = temp_Developed_Country.filter(
        ...
        temp_Developed_Non_Current = temp_Developed_Country.filter(
                    (~pl.col("Internal_Number").is_in(temp_Developed_Current.select(pl.col("Internal_Number")))) &
                    (~pl.col("Internal_Number").is_in(TopPercentage_Securities.select(pl.col("Internal_Number"))))
                )

        # Stack the Frames
        temp_Developed_Country = temp_Developed_Current.vstack(temp_Developed_Non_Current).with_columns(
            pl.lit("Standard").alias("Size"),
            pl.lit(False).alias("Shadow_Company")
        )

        if len(temp_Developed_Country) >= (5 - len(TopPercentage_Securities)):

            # Keep the Securities needed to get the minimum number of Securities
            temp_Developed_Country = temp_Developed_Country.sort("Free_Float_MCAP_USD_Cutoff", descending = True).head(3 - len(TopPercentage_Securities))

            TopPercentage_Securities = TopPercentage_Securities.vstack(temp_Developed_Country.select(TopPercentage_Securities.columns))
...
def Minimum_FreeFloat_Country(TopPercentage, temp_Country, Lower_GMSR, Upper_GMSR, date, country, Segment: pl.Utf8, Original_MCAP_Emerging):
    ...
    if (date == Starting_Date) | (len(Output_Standard_Index.filter(pl.col("Country") == country)) == 0):
        ...
        if len(temp_Developed.filter(pl.col("Country") == country)) >= 3:
            ...
            if len(TopPercentage_Securities.filter(pl.col("Shadow_Company") == False)) < 3:
                ...
                TopPercentage_Securities_Addition = temp_Developed.filter(pl.col("Country") == country).sort("Free_Float_MCAP_USD_Cutoff", descending=True).select(pl.col([
        ...
        # Transform TopPercentage from Companies to Securities level
        TopPercentage_Securities = temp_Developed.select(pl.col("Date", "Internal_Number", "Instrument_Name",
                                "ENTITY_QID", "Country", "Free_Float_MCAP_USD_Cutoff", "Full_MCAP_USD_Cutoff")).filter(pl.col("ENTITY_QID").is_in(TopPercentage.select(
                                    pl.col("ENTITY_QID").unique()
                                ))).with_columns(
                                    pl.lit("Standard").alias("Size")
        ...
        if len(temp_Developed.filter(pl.col("Country") == country)) >= 3:
            ...
            # Check for Index Continuity
            TopPercentage_Securities, TopPercentage = Index_Continuity(TopPercentage_Securities, TopPercentage, "Developed", temp_Developed, country, Standard_Index)
...
def Index_Rebalancing_Box(Frame: pl.DataFrame, SW_ACALLCAP, Output_Count_Standard_Index, Lower_GMSR, Upper_GMSR, country, date, Excel_Recap,  Right_Limit, Left_Limit, Segment: pl.Utf8, writer):
    ...
    QID_Standard_Index = QID_Standard_Index.join(Developed.filter((pl.col("Country")==country) & (pl.col("Date")==date)).select(pl.col("Free_Float_MCAP_USD_Cutoff",
    ...
    for date in Emerging.select(["Date"]).unique().sort("Date").to_series():
        ...
        if date == Starting_Date: 
            ...
            for country in temp_Developed_Aggregate.select(pl.col("Country")).unique().sort("Country").to_series():
                ...
                TopPercentage, TopPercentage_Securities = Minimum_FreeFloat_Country(TopPercentage, temp_Country, Lower_GMSR, Upper_GMSR, date, country, "Developed", Original_MCAP_Developed)
        ...
        # Following Reviews where Index is rebalanced
            ...
            for country in temp_Developed_Aggregate.select(pl.col("Country")).unique().sort("Country").to_series():
                ...
                # If there is no composition, a new Index will be created
                    ...
                    TopPercentage, temp_Country = Index_Creation_Box(temp_Emerging_Aggregate, Lower_GMSR, Upper_GMSR, country, date, Excel_Recap, Percentage, Right_Limit, Left_Limit, "Developed", writer)
                ...
                # Small Index #
                Developed_Small = temp_Developed.filter((~pl.col("Internal_Number").is_in(TopPercentage_Securities.select(pl.col("Internal_Number")))) & (
                    pl.col("Country") == country)).select(
                    pl.col(["Date", "Internal_Number", "Instrument_Name", "ENTITY_QID", "Country", "Free_Float_MCAP_USD_Cutoff", "Full_MCAP_USD_Cutoff"])
                ).with_columns(
                    pl.lit("Small").alias("Size"),
                ...
                Small_Index = Small_Index.vstack(Developed_Small)
...
# Add SEDOL/ISIN
Standard_Index = Standard_Index.join(Developed.select(pl.col(["Date", "Internal_Number", "ISIN", "SEDOL"])), on=["Date", "Internal_Number"], how="left")
...
# Add SEDOL/ISIN
Small_Index = Small_Index.join(Developed.select(pl.col(["Date", "Internal_Number", "ISIN", "SEDOL"])), on=["Date", "Internal_Number"], how="left")
...
# Store the Results
Standard_Index.write_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\V0_SAMCO\Developed\Output\Standard_Index_Security_Level_CNTarget_{CN_Target_Percentage}_" + current_datetime + ".csv")
Standard_Index_Shadows.write_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\V0_SAMCO\Developed\Output\Standard_Index_Security_Level_Shadows_CNTarget_{CN_Target_Percentage}_" + current_datetime + ".csv")
Recap_Count_Standard.write_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\V0_SAMCO\Developed\Output\Recap_Count_Standard_{Percentage}_{Coverage_Adjustment}_" + current_datetime + ".csv")
Recap_Weight_Standard.write_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\V0_SAMCO\Developed\Output\Recap_Weight_Standard_{Percentage}_{Coverage_Adjustment}_" + current_datetime + ".csv")
