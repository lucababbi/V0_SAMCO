...
Time_Series = pl.read_csv(r"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\V0_SAMCO\Last_SAMCO_PR-GR.csv", separator=";").with_columns(
    pl.col("Date").str.strptime(pl.Date, "%m/%d/%Y")
).select(pl.col(["Date", "MSCI_Gross", "SAMCO_Gross"]))
...
monthly_returns = monthly_returns.with_columns(
    (pl.col("MSCI_Gross") / pl.col("MSCI_Gross").shift(1) - 1).alias("Returns_MSCI"),
    (pl.col("SAMCO_Gross") / pl.col("SAMCO_Gross").shift(1) - 1).alias("Returns_SAMCO_Gross"),
).with_columns(
    (pl.col("Returns_MSCI") - pl.col("Returns_SAMCO_Gross")).alias("Difference_SAMCO_Gross")
)

# Function to calculate tracking error for a given period (in months)
def calculate_tracking_error(data, months):
    return data.tail(months).select(pl.col("Difference_SAMCO_Gross").std()).to_numpy()[0][0] * np.sqrt(12)
